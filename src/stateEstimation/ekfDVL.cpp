...
void ekfClassDVL::predictionImuNoVelocity(double xAccel, double yAccel, double zAccel, Eigen::Quaterniond currentRotation,
                                Eigen::Vector3d positionIMU,
                                rclcpp::Time timeStamp) {
    // for saving the current EKF pose difference in
    // currentStateBeforeUpdate is 12x1 [xpos, ypos, zpos, vx, vy, xz, r, p, y, rvel, pvel, yvel]
    ...
    // roll, pitch, yaw updated based on angular velocity
    A(6, 9) = timeDiff;
    A(7, 10) = timeDiff;
    A(8, 11) = timeDiff;
    // Orientation update from angular velocity
    ...
    // explicitly zero dummy velocity states
    state(3) = 0;
    state(4) = 0;
    state(5) = 0;
    
    // update position based on linear aaceleration
    state(0) += 0.5 * localAcceleration(0) * timeDiff * timeDiff;  // or +0.5 * localAccel(0) * dt*dt
    state(1) += 0.5 * localAcceleration(1) * timeDiff * timeDiff;
    state(2) += 0.5 * localAcceleration(2) * timeDiff * timeDiff;
...
void ekfClassDVL::updateIMUNoVelocity(double roll, double pitch, double xAngularVel, double yAngularVel, double zAngularVel,
    ...
    // we only update roll, pitch and angular velocity here
